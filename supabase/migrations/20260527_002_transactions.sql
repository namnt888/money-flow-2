...
-- Central source of truth for all money movement
-- ============================================================
...
create table if not exists public.transactions (
  ...
  type                  text not null check (type in (
                          ...
                          'transfer_out',
                          'transfer_in',
                          'debt',          -- money lent/borrowed
                          'repayment',     -- paying back debt
                          'refund',
                          'installment',   -- installment charge
                          'service'        -- recurring service charge
                        )),
  status                text not null default 'posted'
                          check (status in ('posted', 'pending', 'void')),
  amount                bigint not null check (amount > 0),  -- VND, always positive
  source_account_id     uuid not null references public.accounts(id),
  dest_account_id       uuid references public.accounts(id), -- for transfer only
  category_id           uuid references public.business_categories(id),
  person_id             uuid references public.people(id),   -- for debt/repayment
  notes                 text,
  merchant              text,
  persisted_cycle_tag   text,               -- 'YYYY-MM' for cashback cycle lock
  -- Relations
  original_txn_id       uuid references public.transactions(id), -- for refund/installment
  debt_id               uuid,              -- FK added in migration 004
  installment_plan_id   uuid,              -- FK added in migration 004
  recurring_service_id  uuid,              -- FK added in migration 006
  -- Raw input tracking
  raw_input             text,              -- original CLI / sheet paste
  parsed_by             text,              -- 'llm' | 'manual' | 'import'
...
-- ============================================================
-- TRIGGER: auto update accounts.current_balance on txn change
-- ============================================================
...
begin
  ...
  set
    current_balance = (
      ...
      where t.source_account_id = acc_id
        and t.status = 'posted'
    ),
    updated_at = now()
  where id = acc_id;

  -- Update available_credit for credit accounts
  update public.accounts
